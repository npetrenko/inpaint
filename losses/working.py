...
def build_gan_losses(decoder_model, discriminator_model, true_image_tensor, train_config):
    ...
    def build_disc_gain():
        disc_logits_true = discriminator_model(true_image_tensor)
        disc_logits_false = discriminator_model(decoder_output)
        disc_logits = tf.concat([disc_logits_true, disc_logits_false], axis=0)
    ...
    disc_output = discriminator_model(decoder_output)
